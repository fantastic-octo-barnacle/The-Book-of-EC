...
RTOS 不会自动让系统实时；它只是提供调度、同步和通信机制。正确性仍取决于共享状态、最坏执行时间和优先级设计。
...
从项目实际使用的 RTOS 文档学习 task、queue、mutex、notification 和 ISR-safe API。再学习优先级反转、栈溢出检测、调度时序分析。
...
建立两个任务通过队列传递递增计数；故意改为共享变量后观察错误，再用适当同步原语修复，并能说明选择原因。
