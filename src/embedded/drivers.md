...
“能读一个传感器”与“能在故障、重启、不同板型下可靠读它”之间，差的是接口边界、状态管理和错误处理。
...
先阅读项目中一条完整路径：外设初始化 → 中断/DMA → 缓冲区 → 应用调用。比较寄存器层、HAL/LL 和业务驱动各自负责的内容。
...
为一个 LED、按键或传感器写出最小接口：初始化、读取/设置、错误状态。让上层只包含接口头文件，确认它不依赖芯片寄存器名。
