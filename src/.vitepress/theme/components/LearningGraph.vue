...
const collection = computed(() => props.collection
  ? graph.collections.find((item) => item.id === props.collection)
  : undefined)
...
const visibleNodes = computed(() => {
  ...
  return scopeNodes.value.filter((node) => {
    const searchable = [node.id, node.title, node.summary, ...node.concepts, ...node.technologies].join(' ').toLowerCase()
...
const selectedPrerequisites = computed(() => selected.value?.relations
  .map((relation) => ({ ...relation, node: byId.get(relation.target)! }))
  .filter((relation) => relation.node) ?? [])
const selectedFollowers = computed(() => selected.value
  ? allNodes.filter((node) => node.relations.some((relation) => relation.target === selected.value!.id))
  : [])
...
function removeFilter(kind: 'concept' | 'technology', value: string) {
  if (kind === 'concept') selectedConcepts.value = selectedConcepts.value.filter((item) => item !== value)
...
function graphElements(): ElementDefinition[] {
  ...
  const edges = visibleNodes.value.flatMap((node) => node.relations
...
onMounted(async () => {
  ...
  cy = cytoscape({
    ...
    style: [
      ...
      { selector: 'node[level = "intro"]', style: { 'background-color': '#eaf5f1', 'border-color': '#a9d3c7' } },
      { selector: 'node[level = "core"]', style: { 'background-color': '#eef3fb', 'border-color': '#aec4e6' } },
      { selector: 'node[level = "advanced"]', style: { 'background-color': '#f4effb', 'border-color': '#c7b7e4' } },
      { selector: 'node[level = "integration"]', style: { 'background-color': '#fff5e5', 'border-color': '#e9c989' } },
      {
        selector: 'edge',
        style: {
          width: 1.6,
      ...
      { selector: 'edge[relation = "recommended"]', style: { 'line-color': '#9aa4b2', 'target-arrow-color': '#9aa4b2', 'line-style': 'dashed' } },
      ...
      { selector: 'edge.related', style: { width: 2.6, 'line-color': '#18836f', 'target-arrow-color': '#18836f' } }
    ]
  })
  cy.on('tap', 'node', (event) => { selectedId.value = event.target.id() })
  cy.on('tap', (event) => { if (event.target === cy) selectedId.value = null })
  renderGraph()
})
...
<template>
  <section class="learning-graph" :class="{ 'is-expanded': isExpanded }" aria-label="学习节点依赖图">
    <div class="graph-controls">
      <input v-model="query" type="search" placeholder="搜索节点、概念或技术" aria-label="搜索学习节点">
      <div class="graph-filter-row graph-selects">
        <span>层级</span>
        <button v-for="level in graph.taxonomy.levels" :key="level" :class="{ active: selectedLevels.includes(level) }" type="button" @click="toggleLevel(level)">
          {{ levelLabels[level] }}
        </button>
        <label>
          <span>概念</span>
        ...
        <label>
          ...
          <select v-model="technologyInput" @change="addFilter('technology')">
            ...
            <option v-for="technology in allTechnologies" :key="technology" :value="technology">{{ technology }}</option>
          </select>
        </label>
      </div>
      <div v-if="selectedConcepts.length || selectedTechnologies.length" class="selected-filters">
        <button v-for="concept in selectedConcepts" :key="concept" type="button" @click="removeFilter('concept', concept)">概念：{{ concept }} ×</button>
        <button v-for="technology in selectedTechnologies" :key="technology" type="button" @click="removeFilter('technology', technology)">技术：{{ technology }} ×</button>
      </div>
    </div>
    ...
    <div class="graph-canvas-wrap">
      ...
      <div class="graph-canvas-actions" aria-label="图操作">
        <button type="button" aria-label="重置筛选" title="重置筛选" @click="resetFilters">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19 8a8 8 0 1 0 1.5 7M19 4v4h-4" /></svg>
        </button>
        ...
        <button type="button" :aria-label="isExpanded ? '收起图窗口' : '展开图窗口'" :title="isExpanded ? '收起图窗口' : '展开图窗口'" :aria-pressed="isExpanded" @click="toggleExpanded">
          <svg v-if="!isExpanded" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4H4v4M16 4h4v4M4 16v4h4M20 16v4h-4" /><path d="M9 9h6v6H9z" /></svg>
    ...
    <aside v-if="selected" class="node-drawer" role="complementary" :aria-label="`${selected.title} 详情`" @keydown.esc.window="selectedId = null">
      <div class="node-drawer-content">
        <button class="drawer-close" type="button" aria-label="关闭详情" @click="selectedId = null">×</button>
        ...
        <dl class="node-drawer-meta">
          <div><dt>概念</dt><dd>{{ selected.concepts.join(' · ') }}</dd></div>
          <div><dt>技术</dt><dd>{{ selected.technologies.join(' · ') }}</dd></div>
        </dl>
        <div class="node-drawer-relations">
          <section v-if="selectedPrerequisites.length">
            ...
            <ul>
              <li v-for="relation in selectedPrerequisites" :key="relation.target">
                <button type="button" @click="focusNode(relation.target)">{{ relation.node.title }}</button>
          ...
          <section v-if="selectedFollowers.length">
            <h3>直接后续</h3>
            <ul>
              <li v-for="node in selectedFollowers" :key="node.id"><button type="button" @click="focusNode(node.id)">{{ node.title }}</button></li>
            </ul>
          </section>
        </div>
